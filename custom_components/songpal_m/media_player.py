...
class SongpalDevice(MediaPlayerDevice):
    ...
    async def async_activate_websocket(self):
        ...
        async def _try_reconnect(connect: ConnectChange):
            ...
            while not self._available:
                if self.cancel:
                    _LOGGER.error("cancel reconnects")
                    break
        ...
        self._tsk_1 = self.hass.loop.create_task(listen_events())
    ...
    async def async_update(self):
        ...
        try:
            ...
            if not self._poll:
                self._tsk = self.hass.async_create_task(self.async_activate_websocket())
                await self._tsk
        except SongpalException as ex:
